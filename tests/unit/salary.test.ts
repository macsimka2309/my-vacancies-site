...
describe("formatHourlyRate", () => {
  // «От» — часть смысла, не оговорка: у Самоката и вахты сверху идут бонусы
  // за SLA и за вес заказа, которых в ставке нет. Без «от» «130 ₽/час» рядом
  // с «2100–4000 ₽ за смену» на карточке читается как противоречие — по
  // чистой ставке смена выходит на 16+ часов.
  it("подписывает ставку с «от»", () => {
    expect(formatHourlyRate(205)).toBe("от 205 ₽/час");
  });
  ...
  it("убирает копейки округлением вниз", () => {
    expect(formatHourlyRate(209.23)).toBe("от 209 ₽/час");
    expect(formatHourlyRate(195.2)).toBe("от 195 ₽/час");
    expect(formatHourlyRate(104.8)).toBe("от 104 ₽/час");
  });
  ...
  it("не округляет половину вверх", () => {
    expect(formatHourlyRate(112.5)).toBe("от 112 ₽/час");
  });
