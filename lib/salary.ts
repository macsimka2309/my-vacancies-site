...
/**
 * Витринная подпись ставки: «от 117 ₽/час».
 *
 * «От» — не оговорка, а часть смысла. Число в таблице партнёра — это база:
 * у Самоката и вахты сверху идут бонусы за скорость (SLA) и за вес заказа,
 * которые в `salaryHour` не входят. Без «от» «130 ₽/час» рядом с «2100–4000 ₽
 * за смену» читается как противоречие — по чистой ставке смена выходит на
 * 16+ часов, — хотя оба числа верны, просто про разные части оплаты.
 *
...
export function formatHourlyRate(value: number | null | undefined) {
  return value ? `от ${formatRate(Math.floor(value))} ₽/час` : null;
}
